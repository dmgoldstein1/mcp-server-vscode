...
// Description: MCP standalone server with N|Solid runtime support
...
const NSOLID_APPNAME = process.env.NSOLID_APPNAME || 'mcp-server-vscode';
const NSOLID_TRACING_ENABLED = process.env.NSOLID_TRACING_ENABLED || '0';
const NSOLID_OTLP = process.env.NSOLID_OTLP || '';

// VS Code bridge configuration
const VSCODE_BRIDGE_PORT = process.env.VSCODE_BRIDGE_PORT || '8991';

// Log N|Solid configuration on startup
if (NSOLID_TRACING_ENABLED === '1' || NSOLID_OTLP) {
  console.error(`N|Solid telemetry enabled for: ${NSOLID_APPNAME}`);
  if (NSOLID_OTLP) {
    console.error(`OTLP exporter: ${NSOLID_OTLP}`);
  }
}
...
// Start the server
async function main() {
  // Log N|Solid runtime information
  console.error(`Starting MCP Server: ${NSOLID_APPNAME}`);
  console.error(`VS Code Bridge Port: ${VSCODE_BRIDGE_PORT}`);
  ...
  console.error(`Application: ${NSOLID_APPNAME}`);
  console.error(`Tracing: ${NSOLID_TRACING_ENABLED === '1' ? 'enabled' : 'disabled'}`);
}
